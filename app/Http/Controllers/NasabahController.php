<?php

namespace App\Http\Controllers;

use App\Models\User;
use App\Models\Data_user;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;
use App\DataTables\NasabahDataTable;
use App\Http\Controllers\Controller;
use \Validator;
use Yajra\DataTables\DataTables;
use Yajra\DataTables\QueryDataTable;

class NasabahController extends Controller
{
    public function index(NasabahDataTable $dataTable){
        // return view('nasabah/nasabah',[
        //     'title' => 'Nasabah Terdaftar',
        //     'm_user' => 'active',
        //     'sm_terdaftar' => 'active',
        //     'data' => $this->getAllUser()
        // ]);
        // $nasabah = new NasabahDataTable;
        // dd($nasabah);

        $query = DB::table('data_user as a')
        ->leftJoin('user as b', 'a.id_user','=','b.id_user')
        ->leftJoin('role as c', 'c.id_role', '=', 'b.id_role')
        ->where('a.id_relasi_user', auth()->user()->id_user)
        ->where('a.id_user', '!=', auth()->user()->id_user);

        // $res = $dataTable->queryBuilder($query)->toJson();
        // $a = $dataTable->dataTable($query);
        return $dataTable->render('nasabah.nasabah',[
                'title' => 'Nasabah Terdaftar',
                'm_user' => 'active',
                'sm_terdaftar' => 'active',
                // 'data' => $this->getAllUser()
            ]);

    }

    public function show(){

        return view('nasabah/add_nasabah',[
            'title' => 'Tambah User',
            'm_user' => 'active',
            'sm_terdaftar' => 'active'
        ]);
    }
    
    public function getAllUser(){
        $users = DB::table('data_user as a')
        // ->select(['id', 'name', 'email', 'created_at', 'updated_at'])
        ->leftJoin('user as b', 'a.id_user','=','b.id_user')
        ->leftJoin('role as c', 'c.id_role', '=', 'b.id_role')
        ->where('a.id_relasi_user', auth()->user()->id_user)
        ->where('a.id_user', '!=', auth()->user()->id_user)
        ->get();

        dd($users);

        return Datatables::of($users)->make();
    }

    public function detail_user($user){
        if ($user != auth()->user()->id_user) {
            return redirect()->route('profil')->withErrors([
                'fail' => 'Anda tidak dapat melakukan permintaan ini.',
            ]);
        } 

        $user = Data_user::where('id_user', $user)->first();
        // dd (json_encode($user));
        if ($user) {
            $data = $user;
            return view('profil.edit_profil', [
                'title' => 'Edit Akun',
                'profil' => 'active',
                'user' => auth()->user(),
                'data' => $data
            ]);
        } else {
            return redirect()->route('profil')->withErrors([
                'fail' => 'Anda tidak dapat melakukan permintaan ini.',
            ]);
        }
    }

    public function store(Request $request){
        // dd (auth()->user());
        // dd($d_user);
        // dd($user);
        // dd($request->all());

        $validate = Validator::make($request->all(), [
            'email' => 'required|email:dns',
        ]);

        if ($validate->fails()) {
            // print_r($validate->messages()->get('*'));
            // dd(true);
            $response = [
                "response" => "fail",
                "message" => "Error! Periksa kembali inputan anda."
            ];


        } else {
            try {
                $user = new User;

                if (auth()->user()->id_role == 2) {
                    $user->id_role = '3';
                    $user->email = $request->email;
                    $user->password = bcrypt('admin');
                    $user->tanggal_dibuat = now();
                    $user->status = 'not active';
                }
                $user->save();

                $dataUser = 
    
                $response = [
                    "response" => "success",
                    "message" => "Data berhasil ditambahkan."
                ];

            } catch (\Throwable $e) {
                Log::error("Failed to update : " . $e->getMessage());
                $response = [
                    "response" => "fail",
                    "message" => "Error DB Connection."
                ];
            }
            
        } 
        
        return redirect()->route('nasabah')->with($response);
    }

    public function update(Request $request, Data_user $d_user, User $user){
        // dd (auth()->user());
        // dd($d_user);
        // dd($user);
        // dd($request->all());

        $validate = Validator::make($request->all(), [
            'role' => 'required|in:1,2,3',
            'nama' => 'required',
            'jabatan' => 'required_if:role,1',
            'instansi' => 'required_if:role,1',
            // 'email' => 'required|email:dns|',
            'no_hp' => 'required',
            'jenis_kel' => 'required|in:L,P',
            'tgl_lahir' => 'nullable',
            'alamat' => 'nullable'
        ]);

        if ($validate->fails()) {
            $response = [
                "response" => "fail",
                "message" => "Error! Periksa kembali inputan anda."
            ];
            // dd(false);

        } else {
            // dd(true);
            try {
                $user = Data_user::
                leftJoin('user','user.id_user','=','data_user.id_user')
                ->where('user.id_user','=', auth()->user()->id_user)
                ->first();
    
                $user->nama_user = $request->nama;
                $user->no_hp = $request->no_hp;
                $user->jenis_kel = $request->jenis_kel;
                $user->ttl = $request->tgl_lahir;
                $user->alamat = $request->alamat;
                if (auth()->user()->id_role == 1) {
                    $user->jabatan = $request->jabatan;
                    $user->nama_instansi = $request->instansi;
                }
    
                $user->save();
                $response = [
                    "response" => "success",
                    "message" => "Data berhasil diupdate."
                ];

            } catch (\Throwable $e) {
                Log::error("Failed to update : " . $e->getMessage());
                // dd($e->getMessage());
                $response = [
                    "response" => "fail",
                    "message" => "Error DB Connection."
                ];
            }
            
        } 
        
        return redirect()->route('profil')->with($response);
    }

    
}
